...
export const CustomDatePicker = ({
  ...
  const renderHeader = (props: ReactDatePickerCustomHeaderProps) => {
    ...
    return (
      <div className="pw-flex pw-flex-col pw-gap-2 pw-mb-3">
        {/* Linha com setas e texto Novembro 2025 */}
        <div className="pw-flex pw-items-center pw-justify-between">
          <button
            type="button"
            onClick={decreaseMonth}
            disabled={prevMonthButtonDisabled}
            className="pw-w-7 pw-h-7 pw-flex pw-items-center pw-justify-center pw-rounded-full pw-text-slate-500 hover:pw-bg-slate-100 disabled:pw-opacity-40 disabled:hover:pw-bg-transparent"
          >
            ‹
          </button>

          <span className="pw-text-sm pw-font-medium pw-text-slate-900">
        ...
        {/* selects discretos de mês/ano (funcionalidade pedida) */}
        <div className="pw-flex pw-justify-center pw-gap-2">
          <BaseSelect className="!w-[140px] !min-w-[150px]" value={getMonth(date)} onChangeValue={(e) => changeMonth(Number(e))} options={months.map((month, index) => ({ value: index, label: month as string }))} />
          <BaseSelect className="!pw-w-[60x]" value={getYear(date)} onChangeValue={(e) => changeYear(Number(e))} options={years.map((year) => ({ value: year, label: year.toString() as string }))} />
        </div>
      </div>
    );
  };
  ...
  const calendarContent = useMemo(() => {
    ...
    if (type === 'range') {
      ...
      return (
        <div className="pw-bg-white pw-rounded-lg pw-shadow-md pw-border pw-border-slate-200 pw-w-full pw-p-6">
          <ReactDatePicker
            inline
            selected={startDate}
    ...
    // SINGLE
    return (
      <div className="pw-bg-white pw-rounded-lg pw-shadow-md pw-border pw-border-slate-200 pw-w-full pw-p-6 pw-absolute pw-top-10 pw-left-0 pw-z-50">
        <ReactDatePicker
          inline
